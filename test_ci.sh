#!/usr/bin/env bash
# Local CI check for zlicenser, mirroring .github/workflows/ci.yml.
...
hr
echo -e "${BLD}zlicenser CI  $(date '+%Y-%m-%d %H:%M:%S')${RST}"
echo -e "  $REPO"
hr
...
step "clippy --all-features"      "cd '$REPO' && cargo clippy --workspace --exclude zlicenser-gui --all-features -- -D warnings" || FAILED+=(clippy-all)
step "clippy -p zlicenser --no-defaults" "cd '$REPO' && cargo clippy -p zlicenser --no-default-features -- -D warnings"  || FAILED+=(clippy-none)
step "test"                       "cd '$REPO' && cargo test -p zlicenser"                                              || FAILED+=(test)
step "test --all-features"        "cd '$REPO' && cargo test -p zlicenser --all-features"                               || FAILED+=(test-all)
